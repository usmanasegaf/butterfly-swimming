...
@section('title', 'Absensi Murid untuk Jadwal: ' . $schedule->swimmingCourse->name)

@section('content')
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Absensi Murid</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <p><strong>Lokasi:</strong> {{ $schedule->location->name }} (Lat: <span id="locationLat">{{ $location->latitude }}</span>, Lon: <span id="locationLon">{{ $location->longitude }}</span>)</p>
            <p><strong>Waktu:</strong> {{ \Carbon\Carbon::parse($schedule->start_time_of_day)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->end_time_of_day)->format('H:i') }}</p>
            ...
            <p><strong>Status GPS Guru:</strong> <span id="gpsStatus" class="text-warning">Mencari lokasi...</span></p>
            <p><strong>Jarak ke Lokasi:</strong> <span id="distanceToLocation">N/A</span></p>
        </div>
    </div>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            @if ($murids->isEmpty()) {{-- Menggunakan $murids sesuai relasi Anda --}}
                <p>Tidak ada murid yang terdaftar untuk jadwal ini.</p>
            @else
                <form id="attendanceForm" action="{{ route('guru.schedules.store_attendance', $schedule->id) }}" method="POST">
                    @csrf
                    <input type="hidden" name="teacher_latitude" id="teacherLatInput">
                    <input type="hidden" name="teacher_longitude" id="teacherLonInput">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                @foreach ($murids as $murid) {{-- Menggunakan $murids --}}
                                    <tr>
                                        <td>{{ $murid->name }}</td>
                                        <td>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="attendance_status[{{ $murid->id }}]" id="hadir_{{ $murid->id }}" value="hadir" required>
                                                <label class="form-check-label" for="hadir_{{ $murid->id }}">Hadir</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="attendance_status[{{ $murid->id }}]" id="alpha_{{ $murid->id }}" value="alpha">
                                                <label class="form-check-label" for="alpha_{{ $murid->id }}">Alpha</label>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    <button type="submit" class="btn btn-success mt-3" id="submitAttendanceBtn" disabled>Simpan Absensi</button>
                </form>
            @endif
        </div>
    </div>
...
@section('scripts') {{-- Pastikan layout Anda memiliki section 'scripts' --}}
<script>
