...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                {{-- Penambahan null safety di sini --}}
                @if (empty($murids) || $murids->isEmpty())
                    <div class="alert alert-warning">
                        Tidak ada murid yang terdaftar untuk jadwal ini.
                        <br>
                        **Penting:** Pastikan murid sudah didaftarkan ke jadwal ini melalui tabel pivot `schedule_murid`.
                    </div>
                @else
                    <form id="attendanceForm" action="{{ route('guru.schedules.store_attendance', $schedule->id) }}"
                                            ...
                                            <td>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio"
                                                        name="attendance_status[{{ $murid->id }}]"
                                                        id="hadir_{{ $murid->id }}" value="hadir" required>
                                                    <label class="form-check-label"
                                                        for="hadir_{{ $murid->id }}">Hadir</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio"
                                                        name="attendance_status[{{ $murid->id }}]"
                                                        id="alpha_{{ $murid->id }}" value="alpha">
                                                    <label class="form-check-label"
                                                        for="alpha_{{ $murid->id }}">Alpha</label>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <button type="submit" class="btn btn-success mt-3" id="submitAttendanceBtn" disabled>Simpan
                            Absensi</button>
                    </form>
                @endif
            </div>
