...
<a href="{{ route('guru.murid.create') }}">Tambah Murid ke Bimbingan</a>
    ...
    @forelse($murids as $murid)
        <li>
            ...
            <form action="{{ route('guru.murid.destroy', $murid->id) }}" method="POST" style="display:inline">
                @csrf
                @method('DELETE')
                <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus murid ini dari bimbingan?');">Hapus</button>
            </form>
        </li>
    @empty
