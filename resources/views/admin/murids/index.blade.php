...
@section('title', 'Manajemen Murid Aktif')

@section('content')
    <div class="container-fluid">
        <h1 class="h3 mb-4 text-gray-800">Manajemen Murid Aktif</h1>

        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Filter Murid Aktif</h6>
            </div>
            <div class="card-body">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Daftar Murid Aktif</h6>
            </div>
            <div class="card-body">
                @if (session('success'))
                ...
                @if ($murids->isEmpty())
                    <div class="alert alert-info">
                        Tidak ada murid aktif yang ditemukan dengan filter yang dipilih.
                    </div>
                @else
                            ...
                            <tbody>
                                @foreach ($murids as $murid)
                                    <tr>
                                        ...
                                        <td>{{ $murid->course_assigned_at ? \Carbon\Carbon::parse($murid->course_assigned_at)->format('d M Y') : 'N/A' }}
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Aksi Murid">
                                                {{-- AKTIFKAN KEMBALI @can directive ini --}}
                                                @can('manage active students')
                                                    <a href="{{ route('admin.murids.edit', $murid->id) }}"
                                                        class="btn btn-warning btn-sm">Edit</a>
                                                    <form action="{{ route('admin.murids.destroy', $murid->id) }}"
                                                        method="POST"
                                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus murid ini? Ini akan menghapus semua data terkait murid.')"
