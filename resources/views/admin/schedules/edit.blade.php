...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Edit Jadwal Kursus: {{ $schedule->swimmingCourse->name ?? 'N/A' }}</h1>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('swimming_course_id') is-invalid @enderror" id="swimming_course_id" name="swimming_course_id" required>
                        ...
                        @foreach ($swimmingCourses as $course)
                            <option value="{{ $course->id }}" {{ old('swimming_course_id', $schedule->swimming_course_id) == $course->id ? 'selected' : '' }}>{{ $course->name }} ({{ $course->level }})</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('guru_id') is-invalid @enderror" id="guru_id" name="guru_id" required>
                        ...
                        @foreach ($gurus as $guru)
                            <option value="{{ $guru->id }}" {{ old('guru_id', $schedule->guru_id) == $guru->id ? 'selected' : '' }}>{{ $guru->name }}</option>
                        @endforeach
                    </select>
                    @error('guru_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <input type="time" name="start_time_of_day" id="start_time_of_day" class="form-control @error('start_time_of_day') is-invalid @enderror" value="{{ old('start_time_of_day', \Carbon\Carbon::parse($schedule->start_time_of_day)->format('H:i')) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="time" name="end_time_of_day" id="end_time_of_day" class="form-control @error('end_time_of_day') is-invalid @enderror" value="{{ old('end_time_of_day', \Carbon\Carbon::parse($schedule->end_time_of_day)->format('H:i')) }}" required>
